...
readIni::readIni(QString path)
{
    ...
    {
        qDebug()<<"readIni: cannot open file:"<<path;
        return ;
    }
    filePath=path;
}
...
bool readIni::getNextNumber(float *result)
{
    bool rtn;
    QByteArray rl;
    if(getNextLine(&rl)==true){
        *result=rl.toFloat(&rtn);
        if(rtn==false){
            qDebug()<<"to float failed.";
        }
        return rtn;
    }
    else{
...
bool readIni::changePath(QString newPath)
{
    filePath=newPath;
    return this->reopen();
}
